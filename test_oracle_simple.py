...
# phenomenological_oracle_v5.pyを直接インポートするため、パスを調整
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'src', 'core'))

import phenomenological_oracle_v5
...
def main():
    ...
    現象学的オラクルシステムのテストを開始します...
    """)
    
    try:
        # システムの初期化
        print("1. システムを初期化中...")
        oracle_system = phenomenological_oracle_v5.PhenomenologicalOracleSystem(api_key=api_key)
        print("   ✓ システム初期化完了")
        ...
        # オラクルの表示
        print("\n" + phenomenological_oracle_v5.format_oracle_output(oracle))
        ...
        if simulate_evolution.lower() == 'y':
            ...
            if oracle.imperative:
                ...
                print("\n" + phenomenological_oracle_v5.format_oracle_output(evolved_oracle))
